...
## 步骤 6：run 动画生成（SiliconFlow Wan2.2 图生视频 → 本地抽帧对齐，游戏运行时版）
...
func _frames_from_video(mp4_path: String):
	var frames_dir = _workspace() + "frames"
	...
	var result = AI_HORSE_ENV.run("process_video", [mp4_path, frames_dir, measure_path, str(FRAME_COUNT), str(FRAME_SECONDS)])
	_set_busy(false)
	if result.code == 0 and FileAccess.file_exists(frames_dir + "/1.png"):
