extends Node
## SiliconFlow API 客户端（游戏运行时版，无 @tool）。
## 图生图（风格化）、图生视频（动画）、状态轮询、文件下载。
...
var api_key: String = ""
var workspace_dir: String = "user://ai_horses/workspace/"
...
func generate_image(prompt: String, source_image_path: String, on_success: Callable, on_error: Callable, reference_images: Array = []):
	...
	if not source_image_path.is_empty():
		body["image"] = _data_url(source_image_path)
...
func submit_video(prompt: String, source_image_path: String, on_success: Callable, on_error: Callable):
	var body = {
		"model": VID_MODEL,
		"image": _data_url(source_image_path),
		"prompt": prompt,
	}
...
func _download_via_python(url: String, save_path: String, on_success: Callable, on_error: Callable):
	var python = AI_HORSE_ENV.find_python()
	if python == "":
		on_error.call("找不到 Python 环境，无法下载视频")
		return
	var err = AI_HORSE_ENV.ensure_scripts()
	if not err.is_empty():
		on_error.call(err)
		return
	...
	var script = ProjectSettings.globalize_path("user://ai_horses/py/") + "download.py"
	var output = []
...
## 按文件扩展名生成正确的 data URL（jpg/jpeg → image/jpeg，其余 → image/png）
func _data_url(path: String) -> String:
	var mime = "image/jpeg" if path.get_extension().to_lower() in ["jpg", "jpeg"] else "image/png"
	return "data:%s;base64,%s" % [mime, _file_to_base64(path)]
