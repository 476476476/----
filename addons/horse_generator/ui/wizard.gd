@tool
extends Window
## AI 马匹生成器主向导：7 步流程控制 + 共享状态

const STEP_NAMES = ["① 上传照片", "② 属性配置", "③ 风格化生成", "④ 背景抠除", "⑤ 背高验证", "⑥ 动画生成", "⑦ 完成注册"]

const STEP_SCRIPTS = [
	preload("res://addons/horse_generator/ui/step_upload.gd"),
	preload("res://addons/horse_generator/ui/step_config.gd"),
	preload("res://addons/horse_generator/ui/step_portrait.gd"),
	preload("res://addons/horse_generator/ui/step_bg_remove.gd"),
	preload("res://addons/horse_generator/ui/step_back_verify.gd"),
	preload("res://addons/horse_generator/ui/step_animation.gd"),
	preload("res://addons/horse_generator/ui/step_finalize.gd"),
]

const SILICONFLOW_CLIENT = preload("res://addons/horse_generator/api/siliconflow_client.gd")
const CONFIG = preload("res://addons/horse_generator/config.gd")
...
func _ready():
	title = "AI 马匹生成器 — 让玩家上传照片，生成游戏新品种"
	_config = CONFIG.new()
...
func _build_ui():
	...
	title_label.add_theme_color_override("font_color", Color(0.62, 0.4, 0.16, 1.0))
...
func _refresh_api_status():
	if _config.has_api_key():
		_status_label.text = "✅ SiliconFlow API Key 已配置（%s）" % _config.get_env_path()
		_status_label.add_theme_color_override("font_color", Color(0.2, 0.55, 0.25, 1.0))
	else:
		_status_label.text = "⚠️ 未检测到 API Key：请在 %s 创建 .env 并填入 SILICONFLOW_API_KEY（参考 .env.example）" % _config.get_env_path()
...
func _goto_step(index: int):
	...
	_btn_next.text = "🚀 完成注册" if is_last else "下一步 →"
